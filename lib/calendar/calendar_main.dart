...
class CalendarState extends State<Calendar> with TickerProviderStateMixin {
  ...
  futuro(BuildContext context) async {
    ...
    QuerySnapshot list = await Firestore.instance
        ...
        .getDocuments();
    var lista1 = list.documents.map((doc) => doc.data['data']);
    var lista3 = list.documents.map((doc) => doc.data['hora']);
    var lista2 = lista1.toList();
    print(lista2);
    var size_lista2 = lista2.length;
    print(_events);
    var lista4 = lista3.toList();

    for (var x = 0; x < size_lista2; x++) {
      DateTime tempDate = new DateFormat("dd-MM-yyyy").parse(lista2[x]);
      List eventos = lista4;
      print(tempDate);
      _events[tempDate] = eventos;
    }
